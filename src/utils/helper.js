...
export const getQueryParameter = (parameterName) => {
  // Prefer standard `location.search` when available (query before hash)
  try {
    ...
    // Support query parameters placed after hash: e.g. index.html#wishas?to=Name
    const hash = window.location.hash || "";
    const hashQueryIndex = hash.indexOf("?");
    if (hashQueryIndex !== -1) {
      const hashQuery = hash.substring(hashQueryIndex + 1).split("#")[0];
      const fromHash = new URLSearchParams(hashQuery).get(parameterName);
      if (fromHash) return fromHash;
    }

    // Fallback: parse full href as before
